<template>
  <div>
    <a-divider orientation="center">
      <h1 id="headerregister">{{ $t('masterData.titletcompany') }}</h1>
    </a-divider>
          ...
          <a-row>
            ...
            <a-col class="SearchBox" :xs="{ span: 24 }" :md="{ span: 16 }">
              <a-button
                ...
                icon="plus-square"
                @click="showModal()"
                >{{ $t('masterData.titletIInsertcompany') }}</a-button
              >
            </a-col>
          </a-row>
        </div>
        ...
        <validation-observer ref="inputForm" v-slot="{ handleSubmit }" slim>
          ...
          >
            <a-form :form="form">
              <validation-provider
                v-slot="{ errors }"
                :name="$t('masterData.companyModel.titleNamecompany')"
                rules="required"
              >
                <a-form-item
                  :validate-status="errors[0] ? 'error' : null"
                  :help="errors[0]"
                  :label="$t('masterData.companyModel.titleNamecompany')"
                >
                  <a-input
                    v-model="form.nameTh"
                    :placeholder="
                      $t('masterData.companyModel.titleNamecompany')
                    "
                  ></a-input>
                </a-form-item>
              </validation-provider>
              <validation-provider
                v-slot="{ errors }"
                :name="$t('masterData.companyModel.titleNamecompanyEn')"
                rules="required"
              >
                <a-form-item
                  :validate-status="errors[0] ? 'error' : null"
                  :help="errors[0]"
                  :label="$t('masterData.companyModel.titleNamecompanyEn')"
                >
                  <a-input
                    v-model="form.nameEn"
                    :placeholder="
                      $t('masterData.companyModel.titleNamecompanyEn')
                    "
                  ></a-input>
                </a-form-item>
              </validation-provider>
              <validation-provider
                v-slot="{ errors }"
                :name="$t('masterData.companyModel.titleDescrption')"
                rules="required"
              >
                <a-form-item
                  :validate-status="errors[0] ? 'error' : null"
                  :help="errors[0]"
                  :label="$t('masterData.companyModel.titleDescrption')"
                >
                  <a-input
                    v-model="form.description"
                    :placeholder="$t('masterData.companyModel.titleDescrption')"
                  ></a-input>
                </a-form-item>
              </validation-provider>
              <a-form-item hidden>
...
export default defineComponent({
  setup(_props, context) {
    ...
    const columns = [
      ...
      {
        slotName: 'masterData.companyModel.titleNamecompany',
        dataIndex: 'nameTh',
        scopedSlots: {
          title: 'masterData.companyModel.titleNamecompany',
        },
      },
      {
        slotName: 'masterData.companyModel.titleNamecompanyEn',
        dataIndex: 'nameEn',
        scopedSlots: {
          title: 'masterData.companyModel.titleNamecompanyEn',
        },
      },
      {
        slotName: 'masterData.companyModel.titleDescrption',
        dataIndex: 'description',
        scopedSlots: {
          title: 'masterData.companyModel.titleDescrption',
        },
      },
      {
