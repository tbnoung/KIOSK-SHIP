<template>
  <div>
    <a-divider orientation="center">
      <h1 id="headerregister">{{ $t('masterData.titletobjective') }}</h1>
    </a-divider>
          ...
          <a-row>
            ...
            <a-col class="SearchBox" :xs="{ span: 24 }" :md="{ span: 16 }">
              <a-button
                ...
                icon="plus-square"
                @click="showModal()"
                >{{ $t('masterData.titletIInsertobjective') }}</a-button
              >
            </a-col>
          </a-row>
        </div>
        ...
        <validation-observer ref="inputForm" v-slot="{ handleSubmit }" slim>
          ...
          >
            <a-form ref="formRef" :model="form">
              <validation-provider
                v-slot="{ errors }"
                :name="$t('masterData.ObjectiveModel.titleObjective')"
                rules="required"
              >
                <a-form-item
                  :validate-status="errors[0] ? 'error' : null"
                  :help="errors[0]"
                  :label="$t('masterData.ObjectiveModel.titleObjective')"
                >
                  <a-input
                    v-model="form.nameTh"
                    :placeholder="
                      $t('masterData.ObjectiveModel.titleObjective')
                    "
                  ></a-input>
                </a-form-item>
              </validation-provider>
              <validation-provider
                v-slot="{ errors }"
                :name="$t('masterData.ObjectiveModel.titleObjectiveEN')"
                rules="required"
              >
                <a-form-item
                  :validate-status="errors[0] ? 'error' : null"
                  :help="errors[0]"
                  :label="$t('masterData.ObjectiveModel.titleObjectiveEN')"
                >
                  <a-input
                    v-model="form.nameEn"
                    :placeholder="
                      $t('masterData.ObjectiveModel.titleObjectiveEN')
                    "
                  ></a-input>
                </a-form-item>
              </validation-provider>
...
export default defineComponent({
  setup(_props, context) {
    ...
    const columns = [
      ...
      {
        slotName: 'masterData.ObjectiveModel.titleObjective',
        dataIndex: 'nameTh',
        scopedSlots: {
          title: 'masterData.ObjectiveModel.titleObjective',
        },
      },
      {
        slotName: 'masterData.ObjectiveModel.titleObjectiveEN',
        dataIndex: 'nameEn',
        scopedSlots: {
          title: 'masterData.ObjectiveModel.titleObjectiveEN',
        },
      },
      {
    ...
    const onSubmit = async () => {
      ...
      if (form.id > 0) {
        await context.root.$axios
          .put('/apis/Objectivity/Update', form)
          .then(
            Modal.success({
              title: 'สำเร็จ',
            })
        ...
        await context.root.$axios
          .post('/apis/Objectivity/Create', form)
          .then(
            Modal.success({
              title: 'สำเร็จ',
            })
      ...
      await context.root.$axios
        ...
        .then((r) => {
          const datavalue = r.data.message.filter(
            (x) => x.nameTh === value.trim() || x.nameEn === value.trim()
          )
          // eslint-disable-next-line array-callback-return
          datavalue.map((el) => {
            numRow.value = numRow.value + 1
    ...
    const deleteItem = (id) => {
      ...
      if (id != null) {
        Modal.confirm({
          ...
          onOk() {
            context.root.$axios
              .delete('/apis/Objectivity/Delete/' + id)
              .then(
                Modal.success({
                  title: 'สำเร็จ',
                })
